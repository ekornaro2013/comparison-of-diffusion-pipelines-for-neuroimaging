...
def BuildPipeLine():
        ...
        if not os.path.exists(os.path.join(expDir, '%s_pipe_lpca_gibbs' % nipype_name, outputFolderName)):
                os.makedirs(os.path.join(expDir, '%s_pipe_lpca_gibbs' % nipype_name, outputFolderName))

        # check if index file is avialble, if not create a new one
        eddyIndexTxtFile = os.path.join(expDir, '%s_pipe_lpca_gibbs' % nipype_name, outputFolderName, 'eddy_index.txt')
...
#               f.write(output)
        ...
        preproc.base_dir = os.path.join(expDir, '%s_pipe_lpca_gibbs' % nipype_name)
...
#       preproc.config['execution'] = {'remove_unnecessary_outputs' : 'false', 'keep_inputs' : True, 'remove_unnecessary_outputs' : True, 'stop_on_first_crash' : True}
        ...
        dataSink.inputs.base_directory = os.path.join(expDir, '%s_pipe_lpca_gibbs' % nipype_name)
        ...
        if DWImaskFile==None:
                ...
                preproc.connect([
                        (gibbs, selectCorrectedB, [('out_file', 'in_file')]),
        ...
        preproc.connect([
                ...
                # DENOISING
                (infoSource, lpca, [('dwi_file', 'in_file')]),

                # GIBBS ARTEFACT REMOVAL
                (lpca, gibbs, [('out_file', 'in_file')]),


                # EXTRA BRAIN MASK FOR COMPARSION
                (gibbs, mrtrixBrainMask, [('out_file', 'in_file')]),
                (infoSource, mrtrixBrainMask, [('bval', 'in_bval')]),
                (infoSource, mrtrixBrainMask, [('bvec', 'in_bvec')]),

                # WM PARCELLATION - TractSeg. Version 3 
                # gradcheck
                (gibbs, mrtrixGradCheck, [('out_file', 'in_file')]),
                (infoSource, mrtrixGradCheck, [('bval', 'in_bvals')]),
                (infoSource, mrtrixGradCheck, [('bvec', 'in_bvecs')]),
                # TractSeg
                (gibbs, tractSeg, [('out_file', 'in_file')]),
                (mrtrixGradCheck, tractSeg, [('out_bvals', 'in_bvals')]),
                (mrtrixGradCheck, tractSeg, [('out_bvecs', 'in_bvecs')]),
                (rename, tractSeg, [('out_file', 'in_mask')]),
                (infoSource, diff_noise, [('dwi_file', 'in_file')]),
                (lpca, diff_noise, [('out_file', 'in_file2')]),

                (lpca, compute_snr, [('out_file', 'in_signal')]),
                (diff_noise, compute_snr, [('out_file', 'in_noise')]),
                (rename, compute_snr, [('out_file', 'in_mask')]),
                ...
                (compute_snr, QC_report, [('SNR', 'in_SNR')]),
        ...
        # ---------------------------------------
                ...
                # SELECT FILES
                (lpca, selectDenoised, [('out_file', 'in_file')]),
                (gibbs, selectGibbs, [('out_file', 'in_file')]),

                # LPCA
                (selectDenoised, dataSink, [('roi_file', 'DWIspace.dwi_proc.@lpca')]),
                (diff_noise, dataSink, [('out_file', 'DWIspace.dwi_proc.@noise')]),

                # GIBBS REMOVAL
                (selectGibbs, dataSink, [('roi_file', 'DWIspace.dwi_proc.@gibbs')]),
        ...
        if nbUniqueBvals>2:
                ...
                preproc.connect([
                        ...
                        (gibbs, fwDWI, [('out_file', 'in_file')]),
                        (infoSource, fwDWI, [('bval', 'in_bval')]),
                        (infoSource, fwDWI, [('bvec', 'in_bvec')]),

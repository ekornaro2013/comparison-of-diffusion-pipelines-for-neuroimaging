...
def BuildPipeLine():
        ...
        if not os.path.exists(os.path.join(expDir, '%s_pipe_lpca' % nipype_name, outputFolderName)):
                os.makedirs(os.path.join(expDir, '%s_pipe_lpca' % nipype_name, outputFolderName))

        # check if index file is avialble, if not create a new one
        eddyIndexTxtFile = os.path.join(expDir, '%s_pipe_lpca' % nipype_name, outputFolderName, 'eddy_index.txt')
...
#               f.write(output)
        ...
        preproc.base_dir = os.path.join(expDir, '%s_pipe_lpca' % nipype_name)
...
#       preproc.config['execution'] = {'remove_unnecessary_outputs' : 'false', 'keep_inputs' : True, 'remove_unnecessary_outputs' : True, 'stop_on_first_crash' : True}
        ...
        dataSink.inputs.base_directory = os.path.join(expDir, '%s_pipe_lpca' % nipype_name)
        ...
        if DWImaskFile==None:
                ...
                preproc.connect([
                        (lpca, selectCorrectedB, [('out_file', 'in_file')]),

...
#       preproc.config['execution'] = {'remove_unnecessary_outputs' : 'false', 'keep_inputs' : True, 'remove_unnecessary_outputs' : True, 'stop_on_first_crash' : True}
        ...
        dkifit = pe.Node(HelperInterfaces.DKIfitSmooth(), name='dkifit')
		...
		(dkifit, dataSink, [('out_rk', 'DWIspace.dki.@rk')]),
         ...
         preproc.connect([
                ...
                # DIFFUSION TENSOR RECONSTRUCTION
                # dtifit
                (infoSource, dtismooth, [('dwi_file', 'in_file')]),
                (infoSource, dtismooth, [('bval', 'in_bvals')]),
                (infoSource, dtismooth, [('bvec', 'in_bvecs')]),
                (infoSource, dtismooth, [('dwi_mask', 'in_mask')]),
                (dtismooth, dtifit, [('out_dti', 'dwi')]),
